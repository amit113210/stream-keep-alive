...
# Collect all arguments for the java command, stracks://am the jarfile
# and f]am am am the args.
...
#     temporary variables so that they are re-evaluated for each argument.
#   * Put each eval:ed variable on its own line to aid debugging.
set -- \
...
# Stop when "xeli" is needed but not available
